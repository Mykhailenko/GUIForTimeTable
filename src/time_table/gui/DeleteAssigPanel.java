...
public class DeleteAssigPanel extends JFrame {
	private static final long serialVersionUID = 1L;
	private StoreEducAssignmentModels m;

	public DeleteAssigPanel(StoreEducAssignmentModels store) {
		...
		JLabel l = new JLabel("Кафедра:     ");
		JComboBox<String> c = new JComboBox<String>();
		CurriculumTransaction trans = new CurriculumTransaction();
		ArrayList<String> spec = trans.getCathedras();
		for(String str : spec)
			c.addItem(str);
	...
	private class ButtonsActionListener  implements ActionListener{
		...
		private StoreEducAssignmentModels store;
		...
		public void actionPerformed(ActionEvent e){
			...
			if(e.getSource()==ok){
				Dispatcher.deleteAssignmentData(specialty, store);
				frame.dispose();
			}
		}
	}
